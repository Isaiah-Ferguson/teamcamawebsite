...
import { formatDays, formatTime, programPath, programs } from "../lib/programs";
  ...
  return <div className="grid lg:grid-cols-3 gap-7">
    {programs.map(program => <section key={program.id} className="border-t-2 border-primary pt-5">
      ...
      <ul className="divide-y divide-rule">
        {program.schedule.map(row => <li key={formatDays(row.days) + row.group} className="py-4">
          <div className="flex flex-wrap justify-between gap-x-4 gap-y-1 text-sm font-medium"><span>{formatDays(row.days)}</span><span className="tabular-nums">{formatTime(row)}</span></div>
